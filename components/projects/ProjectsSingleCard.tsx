...
export default function ProjectsSingleCard({ data, isHiddenOnLargeScreen }: { data: ProjectsDataType, isHiddenOnLargeScreen: boolean }) {
    ...
    return (
        <Link
            ...
            className={`flex hover:scale-103 active:scale-103 ${hiddenCls}`}
            key={data.name}
        >
            <motion.div
                className={`flex flex-col gap-2 bg-card-black py-2 border-2 border-edge-yellow rounded-2xl min-h-125`}
