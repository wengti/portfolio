...
export default function ProjectsDoubleCard({ data, isShownOnLargeScreen }: { data: ProjectsDataType, isShownOnLargeScreen: boolean }) {
    ...
    return (
        <Link
            ...
            className={`col-span-2 hover:scale-103 active:scale-103 hidden ${shownCls}`}
            key={data.name}
        >
            <motion.div
                key={data.name}
                className={`flex-col gap-2 bg-card-black py-2 border-2 border-edge-yellow rounded-2xl min-h-125 ${shownCls}`}
